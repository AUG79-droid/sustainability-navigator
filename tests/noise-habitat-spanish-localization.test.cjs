...
test("keeps one bilingual Noise & Habitat identity in catalogue 29", () => {
  ...
  assert.equal(catalogue.resources.filter(item => item.languages.includes("es") && item.languages.includes("en")).length, 21);
...
test("recalculates only the real Spanish path coverage and preserves revisions", () => {
  ...
  assert.deepEqual(pathApi.languageAvailability(inService, catalogue, "es"), { language: "es", status: "partial", availableRequired: 4, totalRequired: 5 });
